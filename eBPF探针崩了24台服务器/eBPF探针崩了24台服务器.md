...
> **TL;DR** 2024 年 10 月，我在测试集群部署了一个基于 eBPF 的观测工具（kindling），想看看性能数据。结果 24 台物理机内核崩溃重启，900 个 Pod 被连带重建。原因不是程序写得差，而是 CentOS 4.18 内核的一个深层 bug：两个 eBPF 程序同时运行时，后结束的那个会污染前一个的执行上下文，导致内核 panic。这事让我彻底改变了对 eBPF 的看法——它不是"装上就完事的黑盒探针"，而是一把在内核里运行的刀。
...
2024 年 10 月 31 日晚上九点。

我想在集群上试试 kindling 这个观测工具——它是基于 eBPF 的，能无侵入地观测网络延迟、系统调用等指标，很适合做性能分析。

我当时选的是 **jiagou_wcloud_offline_binhai**（测试集群），灰度了 31 台机器。
...
然后第二台，第三台……到晚上十一点，一共 **24 台物理机陆续内核 panic 重启**。
...
t1: eBPF 程序 A（安全探针）在内核中运行，正在处理某个事件
t2: eBPF 程序 B（kindling 探针）被加载，触发了同一个 hook
t3: 程序 B 执行完毕
    → 正常情况下，B 的上下文应该被清理干净
    → 但 4.18 内核有 bug：B 占用的上下文没有被完全释放
t4: 程序 A 恢复执行
    → A 读到了 B 留下的上下文——是脏数据
    → 执行现场完全错乱
    → 访问到非法内存地址
...
现在每次我要引入一个新的 eBPF 工具时，都会先问自己：

- 当前内核版本是多少？（低于 5.x 要格外小心）
- 这台机器上已经有什么 eBPF 程序在跑了？
- 停掉这个探针，我有恢复手段吗？
- 如果它崩了，我怎么知道是它崩的？（eBPF 崩了就是 kernel panic，日志很容易被淹没在系统重启的噪音里）
