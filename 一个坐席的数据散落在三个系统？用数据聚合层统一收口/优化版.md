# 数据散落在三个系统？用数据聚合层（Layer 模式）统一收口
...
```mermaid
flowchart TD
    A[Controller / 业务编排层] --> B[UserOrgLayerService]
    A --> C[PhoneBarLayerService]
    A --> D[DirectLineLayerService]
    B --> E["HR/OA 系统 (RPC)"]
    C --> F["云电话系统 (RPC)"]
    D --> G["直线号码系统 (RPC)"]
...
## ✅ Layer 模式落地 Checklist

- [ ] **每个外部系统一个 Layer Service？** 不要在业务代码里直接调外部 RPC
- [ ] **重试 + 监控打点统一了？** 读操作大胆重试，写操作谨慎
- [ ] **故障隔离了吗？** Layer 失败返回 null/默认值，不抛异常拖垮全局
- [ ] **数据源变更只需改一个文件？** 调用者不关心数据从哪来
- [ ] **缓存兜底有了吗？** 连续失败时从缓存返回，不让前端拿到空值
- [ ] **缓存一致性兜底？** 消息可能丢，定时全量同步 + 手动刷新运维接口
